...
{
    uint32_t read_us, now_pos;
    stk_time_t timestamp;

    /* Read some track data if there is buffer space. */
    timestamp = stk_now();
    now_pos = (dma_rd->state == DMA_active)
        ? max_t(int32_t, 0, stk_delta(index.prev_time, timestamp)) : sync_pos;
    now_pos %= stk_ms(DRIVE_MS_PER_REV);
    now_pos *= SYSCLK_MHZ / STK_MHZ;
